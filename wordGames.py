#Problem Set 5
#An implementation of a word game described in the problem set. 
...
def is_valid_word(word, hand, word_list):
    ...
    if word in word_list:
        for key in word_dict:
            #print(word_dict.get(key,0),hand.get(key,0))
            if word_dict.get(key,0) > hand.get(key,0):
                is_valid = False
                return is_valid
            else:
                is_valid = True
...
def play_hand(hand, word_list):
    ...
    * The final score is displayed.
    ...
    while len(updated_hand) > 0:
        ...
        while True:
            word = input("Enter your word. To exit, please press .")
            if word == ".":
                return total
            if len(word) > HAND_SIZE or is_valid_word(word,updated_hand,word_list) == False:
